...
BR_fileLoc<-"Y:/Lacher/IaraSpatialLayers_HF/PreparedRasters/ProLands/BlueRidge/"# HF
BR_fileLoc<-"V:/IaraSpatialLayers/PreparedRasters/ProLands/" #V:
...
# ----------------------------------------------
# TOTAL AREA OF NLCD CLASSES IN STUDY AREA (BLUE RIDGE AND PIEDMONT)
...
print(u_vals[categ])		
	...
	ras_patch[[n_p]]$area.ha<-ras_patch[[n_p]]$count*32400/10000#Makes sure resolution is right here. 30*30=900, 180*180=32400, 360*360=129600
	ras_patch[[n_p]]$Raster <- paste0(names(pl_nlcd))
	ras_patch[[n_p]]$Class <- paste0(u_vals[categ])
		n_p <- n_p +1
}
ras_patches<- bind_rows(ras_patch)
colnames(ras_patches) <- c("er", "count", "NLCD_area_ha", "Raster", "Class")
er_nlcd <- ras_patches


# WRITE TO FILE
write.table(er_nlcd, file = paste0(Output_Folder,"er_nlcd",".txt"), row.names=FALSE, sep=",")
# READ TO FILE
...
# ----------------------------------------------
# TOTAL AREA OF RESILIENCE CLASSES IN STUDY AREA (BLUE RIDGE AND PIEDMONT)
# ** consider masking pl_er to only 3 regions in study area. right now does all 10.
# ----------------------------------------------
{
...
# ----------------------------------------------
# MAJORITY VALUES FOR GAP, TYPE, ECOREGION, OWNERSHIP, NLCD: 
# !! This is not longer used for area calcs!!
# Is useful for other things, however... 
# ----------------------------------------------
{
...
# ----------------------------------------------
# ACTUAL AREA VALUES FOR GAP, OWNERSHIP, NLCD, RESILIENCY
# ----------------------------------------------
{

rasters <- c(pl_pp, pl_gap, pl_own, pl_nlcd, pl_resil)
names(rasters)<-c("pl_pp", "pl_gap", "pl_own", "pl_nlcd", "pl_resil")
...
colnames(ras_patches[[1]]) <- c("patchID", "pp_count", "pp_area_ha", "pp_Class")
colnames(ras_patches[[2]]) <- c("patchID", "gap_count", "gap_area_ha", "gap_Class")
colnames(ras_patches[[3]]) <- c("patchID", "own_count", "own_area_ha", "own_Class")
colnames(ras_patches[[4]]) <- c("patchID", "nlcd_count", "nlcd_area_ha", "nlcd_Class")
colnames(ras_patches[[5]]) <- c("patchID", "resil_count", "resil_area_ha", "resil_Class")

# WRITE TO FILE
write.table(ras_patches[[1]], file = paste0(Output_Folder,"pp_area",".txt"), row.names=FALSE, sep=",")
# READ TO FILE
pp_area<-read.table(paste0(Output_Folder,"pp_area",".txt"), sep=",", header=TRUE)
 
# WRITE TO FILE
write.table(ras_patches[[2]], file = paste0(Output_Folder,"gap_area",".txt"), row.names=FALSE, sep=",")
# READ TO FILE
gap_area<-read.table(paste0(Output_Folder,"gap_area",".txt"), sep=",", header=TRUE)

# WRITE TO FILE
write.table(ras_patches[[3]], file = paste0(Output_Folder,"own_area",".txt"), row.names=FALSE, sep=",")
# READ TO FILE
own_area<-read.table(paste0(Output_Folder,"own_area",".txt"), sep=",", header=TRUE)

# WRITE TO FILE
write.table(ras_patches[[4]], file = paste0(Output_Folder,"nlcd_area",".txt"), row.names=FALSE, sep=",")
# READ TO FILE
nlcd_area<-read.table(paste0(Output_Folder,"nlcd_area",".txt"), sep=",", header=TRUE)

# WRITE TO FILE
write.table(ras_patches[[5]], file = paste0(Output_Folder,"resil_area",".txt"), row.names=FALSE, sep=",")
# READ TO FILE 
resil_area<-read.table(paste0(Output_Folder,"resil_area",".txt"), sep=",", header=TRUE)

#QC:
# str(pp_area); str(gap_area); str(own_area); str(nlcd_area); str(resil_area)
# 'data.frame':	8310 obs. of  4 variables:
 # $ zone   : int  1 11 17 49 64 69 97 120 145 158 ...
 # $ count  : int  151 19 57 272 48 365 349 190 21 503 ...
...
bindC <- cbind(nlcd_area, resil_area)
bindC <- bindC[,c(1:4,6:8)]

bindB <- cbind(pp_area, bindC)
bindB <- bindB[,c(1:4,6:11)]
...
# 'data.frame':	132960 obs. of  18 variables:
 ...
 # $ pp_count     : int  151 0 151 0 151 0 151 0 151 0 ...
 # $ pp_area_ha   : num  489 0 489 0 489 ...
